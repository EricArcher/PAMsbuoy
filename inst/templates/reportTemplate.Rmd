...
Navy surplus sonobuoys (type-`r sonobuoyType`) were deployed from the survey vessel during `r cruiseAbbr`; this report includes deployments conducted between `r format(min(detSummary$UTC), format='%b %d %Y')` and `r format(max(detSummary$UTC), format='%b %d %Y')` on the vessel(s) `r formatListGrammar(myVessels)`. Sonobuoys were programmed to allow for best performance (Table 1). Sonobuoy signals were received via shipboard antenna cabled to a Winradio sonobuoy receiver (model WR-G39WSBE). Recordings were digitized at a `r round(stationInfo$sampleRate/1000, 0)`
kHz sampling rate with a `r stationInfo$recordingSystem` and recorded to hard drive using Pamguard software (Pamguard version xxxx, Gillespie et al. 2008). 
...
A total of `r nSonobuoys` sonobuoys were deployed at `r nrow(distinct(detSummary, Station))` locations throughout the study area (Fig. 1)`r vesselText` (Table 1).  Deployments included `r nStations` sonobuoy stations and `r nOpportunistic` sonobuoys deployed opportunistically in the presence of sighted cetaceans (Table 1). 
`r if(nOpportunistic > 0) paste0('Opportunistic sonobuoys included a total of ', round(opportunisticLength/60, 0), ' minutes of recordings during confirmed visual sightings of ', formatListGrammar(oppSpecies), ' (Table 1).')` Acoustic detections included species name 
...
```{r echo=FALSE, fig.cap = stationCap}
knitr::include_graphics(paste0(outDir, '/Figures/stationPlot.jpeg'))
```


```{r echo=FALSE, fig.cap = opportunisticCap}
if(nOpportunistic > 0) {
...
```{r echo=FALSE, fig.cap = detectionCap}
